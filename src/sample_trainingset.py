...
fstr = open("../../str_list0","w")
if N_high != len(dat_tot):
    fstr.write("[iteration" + iteration+"-low: 2.0]\n")
for i in range(len(dat_tot)):
    if dat_tot[i][4] - dat_tot[i][2] <= float(E_limit*tot_atom_num):
        fstr.write(absolute_path+"_iteration"+iteration+"/one_shot/OUTCAR"+str(dat_tot[i][3])+"  ::1000000\n")

if N_high != 0:
    fstr.write("[iteration" + iteration+"-high: 0.2]\n")
for i in range(len(dat_tot)):
    if dat_tot[i][4] - dat_tot[i][2] > float(E_limit*tot_atom_num):
        fstr.write(absolute_path+"_iteration"+iteration+"/one_shot/OUTCAR"+str(dat_tot[i][3])+"  ::1000000\n")
