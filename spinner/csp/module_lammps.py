...
import os
import math
import spinner.csp.module_spacegroup
...
def get_latt_coor(tot_atom_num,lmp):
    ...
    a1 = lmp.extract_variable("a1")
    b1 = lmp.extract_variable("b1")
    b2 = lmp.extract_variable("b2")
    c1 = lmp.extract_variable("c1")
    c2 = lmp.extract_variable("c2")
    c3 = lmp.extract_variable("c3")
...
def run_lammps(Emin, tolerance_matrix, atomnamelist, atomnumlist, inp_file, accurate_potential, Vmin):
    ...
    if 'lammps_simd' in inp_file.keys() and inp_file['lammps_simd'] == True:
        try:
            ...
            print("Check whether SIMD version LAMMPS are installed")
            print("If you intend to run with normal version, please make [lammps_simd: False] in yaml file")
    else:
        try:
            #lmp = lammps()
    ...
    del lmp
    status = ""
    success, latt, coor, mindis, etc = check_tolerance(latt, coor, tolerance_matrix, atomnumlist, 0, 1.0, inp_file,0,v,Vmin,100) 

    if success == 0:
        status = "distance cut in the first relax" + etc
        e = 10000.0
        ...
        if accurate_potential  == True:
            ...
            lmp.close()
            del lmp
            success, latt, coor, mindis, etc = check_tolerance(latt, coor, tolerance_matrix, atomnumlist, 0, 1.0, inp_file,1,v,Vmin,5)  
        else:
            success, latt, coor, mindis, etc = check_tolerance(latt, coor, tolerance_matrix, atomnumlist, 0, 1.0, inp_file,1,v,Vmin,4)  
        if success == 0:
            status = "distance cut in the second relax" + etc
            e = 10000.0
        if success == -1:
