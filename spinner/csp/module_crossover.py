...
import random
import math
from spinner.csp.module_lammps import *
import lammps
...
def cut_the_slab(coor, latt, ae, atomnumlist, average_atom_e, latt_direction, inp_file):
    ...
    if sum(weights) == 0.0:
        with open("/data2/wjdwltn928/spinner/error_message","a") as fw:
           fw.write(str(case))
...
#    x2 = np.linalg.norm(latt2)
    x1 = np.linalg.norm(latt1[0])
    x2 = np.linalg.norm(latt2[0])
...
#            if coor1[i][j] > 1.0:
#                coor1[i][j] -= 1.0
            if coor1[i][j] > 1.0 or coor1[i][j] < 0.0:
                coor1[i][j] -= np.floor(coor1[i][j])

    # additional procedure    
    for j in range(3):
        for i in range(N2):
            if coor2[i][j] > 1.0 or coor2[i][j] > 0.0:
                coor2[i][j] -= np.floor(coor2[i][j])
...
def add_atoms(coor, latt, atomnumlist_difference, atomnumlist, atomnamelist, tolerance_matrix, inp_file, interface):
    ...
    for n in range(N):
        ...
        # COOR_1: cartesian nested list
        # COOR_f: fractional nested list
        COOR_f = np.dot(np.copy(COOR_l), np.linalg.inv(latt))

        # original
#        E = evaluate_energy(latt, COOR_l, TYPE, atomnumlist, atomnamelist, lmp, n)
        E = evaluate_energy(latt, COOR_f, TYPE, atomnumlist, atomnamelist, lmp, n)
