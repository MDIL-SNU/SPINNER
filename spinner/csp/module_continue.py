...
def getting_global_best_results(continue_, inp_file, output_dir):
...
def getting_info_from_files(continue_, tot_atom_num, original_path, num_of_atom):
...
def getting_history(original_path, continue_):
    os.chdir(original_path)
    with open("totalGen","r") as f:
        freadlines = f.readlines()
...
def getting_pop_num(continue_, output_dir):
    os.chdir(output_dir)
    f = open("totalGen","r")
    freadlines = f.readlines()
    f.close()
