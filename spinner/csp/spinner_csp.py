...
from spinner.csp.module_input  import *
#from module_log import * 
from spinner.csp.module_pre import *
from spinner.csp.module_post import *
from spinner.csp.module_continue import *
from spinner.csp.module_time  import *
from spinner.csp.module_clean import *
from spinner.csp.module_corejob import *
from spinner.csp.module_structure import *
...
def main(args=None):
    ...
    original_path      = cwd+'/'+original_dir
    ...
    if rank == 0:
      if os.path.isdir(output_path) and continue_ == 0:
        #with open("ERROR_"+input_file, "w") as fw:
        with open("ERROR_output_dir", "w") as fw:
    ...
    if rank == 0:
      if os.path.isdir(input_path) == False:
        with open("ERROR_input_dir", "w") as fw:
    ...
    if rank == 0:
        ...
        shutil.copy(input_path+'/potential', output_path)
        if accurate_potential == True:
            shutil.copy(input_path+'/potential_accurate', output_path)
        if continue_ == 0 or (continue_ !=0 and output_path != original_path):
        ...
        #making folder & energy V log
            ...
            oldresults, pop_info, linelist = getting_info_from_files(continue_, tot_atom_num, original_path, num_of_atom)
            history = getting_history(original_path,continue_)
        ...
        # preprocessing
            ...
            if pop_num > inp_file['structure']['population_max'] and gen > startnum:
                for process in range(1,corenum):
                    ...
                    comm.send([-2, -2,-2,-2,-2,-2,-2,-2,-2,-2,-2,-2], dest=process, tag=1)
                with open(output_path+"/timelog","a") as ftimelog:
                    ftimelog.write("\n")
            ...
            Vmin, pop_info,random_num= get_pop_info(inp_file, gen, pop_num, oldresults, Emin, num_of_best, output_path, ANTISEED)    # pop_info: heredity, permutation, ancestor
        ...
        # Post processing: check if same structure + antiseed
            ...
            while True:
                ...
                if job == 1:  ## lammps calculation
                    t_struct1, t_lmp, t_rdf, t_get, E, V, poscar, contcar, rdfs, latt, coor, atom_e, status, attempt = core_job(gen, rank, rpop, rparent, rstruct_type, rVmin, inp_file, output_path, atom_names, atom_nums,parent_data,continue_, Emin, tolerance_matrix, average_atom_e, startnum)
    ...
    #if rank == 0:
    #    with open(cwd+'/STATUS', 'w') as s:
    #        if len(sys.argv) > 2:
    #            s.write('csp_iteration%s Done'%current_iteration)
    #        else:
    #            s.write('final_csp Done')
