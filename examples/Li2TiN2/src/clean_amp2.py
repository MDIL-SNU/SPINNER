...
fstr = open("../../str_list0","a")

if len(a) != 0:
    fstr.write("\n[iteration"+iteration+"-relax: 20.0]\n")

for i in range(1,num_sample+1):
    for j in range(1,10000):
        if os.path.isfile(str(i)+'/relax_GGA/OUTCAR'+str(j)):
            shutil.copy(str(i)+'/relax_GGA/OUTCAR'+str(j),'../../relax/OUTCAR'+str(i)+'-'+str(j))
            fstr.write(absolute_path+"_iteration"+iteration+"/relax/OUTCAR"+str(i)+'-'+str(j)+"  ::100000\n")
        else:
            break
